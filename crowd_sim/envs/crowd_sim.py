...
class CrowdSim(gym.Env):
    def __init__(self):
        ...
        with open("/home/sriyash421/PRL22/CrowdNav_DSRNN/crowd_sim/envs/points.yaml", "r") as fin:
            self.points = yaml.load(fin, yaml.FullLoader)
        self.point_row = None
    ...
    def configure(self, config):
        ...
        if self.config.humans.policy == 'orca' or self.config.humans.policy == 'social_force' or self.config.humans.policy == 'real_human':
            ...
            self.group_human = None#config.sim.group_humans
        else:
    ...
    def generate_random_human_position(self, human_num):
        ...
        elif self.test_setting == "real_world":
            for i in range(human_num):
                self.humans.append(self.generate_real_human(i))
        else:
            for i in range(human_num):
                self.humans.append(self.generate_circle_crossing_human())
    
    def generate_real_human(self, i):
        arr1 = (0, 4.75, 3.6, 0)
        arr2 = (3.6, 0, 0, 4.75)
        ...
        if i == 2:
            ...
            if self.episode % 2 == 0:
                px, py, gx, gy = (0, 0, 3.6, 4.75)
            else:
                px, py, gx, gy = (3.6, 4.75, 0, 0)
        else:
            if self.episode % 2 == 0:
                px, py, gx, gy = arr2
            else:
        ...
        human.policy.reset(i, self.episode)
        return human
    ...
    def generate_cooperative_human(self, i):
        ...
        if self.point_row is None:
            self.point_row = 0

        px, py = self.points[f'human_{i+1}']['start'][self.point_row]
        gx, gy = self.points[f'human_{i+1}']['goal'][self.point_row]
        human = Human(self.config, 'humans')
        human.set(px, py, gx, gy, 0, 0, 0)
        self.point_row += 1
        return human
